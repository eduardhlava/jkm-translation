          ...
          <div className="flex items-center gap-3 justify-end flex-1">
            ...
            {isAdmin && (
              <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                <Link to="/settings" aria-label={t(ui, "settings")}>
                      ...
                      <TableRow key={it.id}>
                        ...
                        <TableCell className="w-[1%] whitespace-nowrap px-2 py-3 align-top text-center">
                          <div className="flex flex-col items-center gap-1.5 relative">
                            ...
                            {statusHint?.id === it.id && (
                              <div className={`absolute left-full top-1/2 -translate-y-1/2 ml-2 z-30 whitespace-nowrap px-2.5 py-1.5 rounded-md text-xs shadow-md transition-opacity duration-300 pointer-events-none border bg-background text-foreground ${statusHint.type === "confirm" ? "border-success" : "border-destructive"} ${statusHint.visible ? "opacity-100" : "opacity-0"}`}>
                                Nezapomeň potvrdit tlačítkem Aktualizovat
                                <div className={`absolute top-1/2 -left-1 -translate-y-1/2 w-2 h-2 rotate-45 bg-background border-l border-b ${statusHint.type === "confirm" ? "border-success" : "border-destructive"}`} />
                              </div>
                            )}
                          </div>
                        </TableCell>
