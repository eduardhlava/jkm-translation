...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "uploadPdf") {
      const pageId: string = body.pageId;
      const filename: string = (body.filename ?? "dokument.pdf").replace(/[\\/]+/g, "-");
      ...
      if (!upRes.ok) throw new Error(`PDF upload failed [${upRes.status}]: ${await upRes.text()}`);
      ...
      const fileUrl = `${SUPA_URL}/storage/v1${signedPath}`;

      // Read existing files in "soubor" to append
      const pageRes = await notionFetch(`https://api.notion.com/v1/pages/${pageId}`, {
        headers: { Authorization: `Bearer ${NOTION_API_KEY}`, "Notion-Version": NOTION_VERSION },
      }, "page read");
      const page = await pageRes.json();
      const existing = page?.properties?.soubor?.files ?? [];
      // Keep only files not generated by us (drop previous PDFs with same filename)
      const filtered = existing.filter((f: any) => f?.name !== filename);

      const newFile = { name: filename, external: { url: fileUrl } };
      const updatedFiles = [...filtered, newFile];
      ...
      const patchRes = await notionFetch(`https://api.notion.com/v1/pages/${pageId}`, {
        ...
        body: JSON.stringify({ properties: { soubor: { files: updatedFiles } } }),
      }, "page patch soubor");
      if (!patchRes.ok) throw new Error(`Notion patch failed [${patchRes.status}]: ${await patchRes.text()}`);

      return new Response(JSON.stringify({ ok: true, url: fileUrl, filename }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
